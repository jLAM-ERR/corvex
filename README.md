...
| `stop` | Stop xray first; only if that succeeds, disable system proxy and stop AWG tunnel if running. A failed stop (e.g. xray owned by another user) leaves the proxy untouched |
